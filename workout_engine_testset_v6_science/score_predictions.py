...
def main():
    ...
    print(f"Main lift load agreement: {agree}/{total} = {agree/total if total else 0:.3f}")
    print(f"Main lift MAE (point label): {mae:.2f} lb")
    print(f"Decision accuracy: {decision_ok}/{decision_total} = {decision_ok/decision_total if decision_total else 0:.3f}")
    print("Per-user:")
    for u, s in sorted(per_user.items()):
        ...
        print(f"  {u}: agree {agree_rate:.3f}, MAE {user_mae:.2f} lb, decisionAcc {dec_rate:.3f}, n={s['total']}")
