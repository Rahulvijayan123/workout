...
def idx_pred(lst):
    d = defaultdict(list)
    for x in lst:
        d[x.get("lift")].append(x)
        pe = x.get("performed_exercise")
        if pe:
            d[f"performed:{pe}"].append(x)
    return d

def in_range(x, lo, hi): return (x >= lo) and (x <= hi)

def match_pred(e, pred):
    lift = e.get("lift")
    pe = e.get("performed_exercise")
    sub = e.get("substituted_to")
    if lift in pred and pred[lift]: return pred[lift][0]
    if pe and f"performed:{pe}" in pred and pred[f"performed:{pe}"]: return pred[f"performed:{pe}"][0]
    if sub:
        if sub in pred and pred[sub]: return pred[sub][0]
        if f"performed:{sub}" in pred and pred[f"performed:{sub}"]: return pred[f"performed:{sub}"][0]
    return None
...
def main():
    ...
    for r in ds:
        ...
        if k not in pred_map:
            continue
        pred_list = pred_map[k].get("engine_output", {}).get("session_prescription_for_today", [])
        pred = idx_pred(pred_list)

        for e in exp_list:
            if e.get("lift") not in MAIN:
                continue
            p = match_pred(e, pred)
            if not p:
                continue
            ...
            lo, hi = e.get("acceptable_range_lb", [e_w, e_w])
    ...
    print(f"MAE vs point label: {mae:.2f} lb")
    ...
    for u, s in sorted(per_user.items()):
        ...
        print(f"  {u}: agree {s['agree']/s['total'] if s['total'] else 0:.2f}, MAE {user_mae:.2f} lb, decisionAcc {s['decision_ok']/s['decision_total'] if s['decision_total'] else 0:.2f}, n={s['total']}")
