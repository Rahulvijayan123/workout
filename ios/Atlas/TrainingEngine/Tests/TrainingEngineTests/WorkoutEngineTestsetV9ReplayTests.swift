// WorkoutEngineTestsetV9ReplayTests.swift
// Full E2E replay test for workout_engine_testset_v9_conservative_science dataset.
// V9: Conservative progression, exercise-aware (isolation vs compound), cut mode, microloading.
//
...
final class WorkoutEngineTestsetV9ReplayTests: XCTestCase {
    
    // MARK: - V9 Record Structures
    
    struct V9Record: Codable {
        ...
        let input: V9Input
        let expected_output: V9ExpectedOutput
        let assertions: [V9Assertion]?
    }
    
    struct V9Input: Codable {
        let user_profile: V9UserProfile
        let equipment: [String]?
        let equipment_config: V9EquipmentConfig
        let today_metrics: V9TodayMetrics
        let today_session_template: [V9TemplateExercise]
        let recent_lift_history: [String: [V9HistoryEntry]]
        let lift_state: [String: V9LiftState]?
        let days_since_last_session: Int?
        let days_since_lift_exposure: [String: Int?]
        let event_flags: V9EventFlags
        let config_overrides: V9ConfigOverrides?
    }
    
    struct V9UserProfile: Codable {
        let sex: String
        let age: Int
        let height_cm: Int
    ...
    struct V9EquipmentConfig: Codable {
        let units: String
        let bar_weight_lb: Double
        let load_step_lb: Double
    }
    
    struct V9TodayMetrics: Codable {
        let body_weight_lb: Double?
    ...
    struct V9TemplateExercise: Codable {
        let lift: String
        let sets: Int
        let reps: Int
    }
    
    struct V9HistoryEntry: Codable {
        let date: String
    ...
    struct V9LiftStateWeight: Codable {
        let value: Double
        let unit: String
    }
    
    struct V9LiftState: Codable {
        let exerciseId: String
        let lastWorkingWeight: V9LiftStateWeight
    ...
    struct V9EventFlags: Codable {
        let missed_session: Bool
        let injury_flags: [String: V9InjuryFlag]?
    ...
    enum V9InjuryFlag: Codable {
        ...
        init(from decoder: Decoder) throws {
            ...
            throw DecodingError.typeMismatch(V9InjuryFlag.self, .init(codingPath: decoder.codingPath, debugDescription: "Expected Bool or String"))
        }
    ...
    struct V9ConfigOverrides: Codable {
        let progression_policy: String?
        let deload_config: V9DeloadConfig?
        let sex_bw_scaling_enabled: Bool?
    }
    
    struct V9DeloadConfig: Codable {
        let intensityReduction: Double?
        let volumeReduction: Int?
        let failuresBeforeDeload: Int?
    }
    
    struct V9ExpectedOutput: Codable {
        let session_prescription_for_today: [V9Prescription]
    }
    
    struct V9Prescription: Codable {
        let lift: String
    ...
    struct V9Assertion: Codable {
        let field: String
        let expected: V9AssertionValue?
        let expected_range: [Double]?
    }
    
    enum V9AssertionValue: Codable {
        case string(String)
        case number(Double)
        case bool(Bool)
        ...
        init(from decoder: Decoder) throws {
            ...
            if let b = try? container.decode(Bool.self) { self = .bool(b); return }
            throw DecodingError.typeMismatch(V9AssertionValue.self, .init(codingPath: decoder.codingPath, debugDescription: "Unknown type"))
        }
    ...
    struct V9Manifest: Codable {
        let version: String
        let created: String
        let files: [V9ManifestFile]
        let total_cases: Int
        let tags: [String]
    }
    
    struct V9ManifestFile: Codable {
        let path: String
        let count: Int
    }
    ...
    // V9 includes more lifts than just the main 4
    let mainLifts: Set<String> = ["squat", "bench", "deadlift", "ohp", "row", "lat_pulldown", "rdl", "incline_db", "lateral_raise", "bicep_curl", "tricep_pushdown", "leg_curl", "leg_extension"]
    let compoundLifts: Set<String> = ["squat", "bench", "deadlift", "ohp", "row", "lat_pulldown", "rdl", "incline_db"]
    ...
    func testWorkoutEngineTestsetV9_FullE2EReplay() throws {
        let basePath = "/Users/rahulvijayan/Atlas 3/workout_engine_testset_v9_conservative_science"
        let manifestPath = "\(basePath)/manifest.json"
        
        guard let manifestData = try? Data(contentsOf: URL(fileURLWithPath: manifestPath)),
              let manifest = try? JSONDecoder().decode(V9Manifest.self, from: manifestData) else {
            XCTFail("Failed to read manifest.json")
            return
        }
        
        let expectedTotalCases = manifest.total_cases
        print("📋 V9 Manifest: \(manifest.files.count) scenario files, \(expectedTotalCases) expected cases")
        print("📌 V9 Tags: \(manifest.tags.joined(separator: ", "))")
        
        let decoder = JSONDecoder()
        var allRecords: [(scenario: String, record: V9Record)] = []
        ...
        for fileEntry in manifest.files {
            ...
            for (idx, line) in lines.enumerated() {
                ...
                do {
                    let record = try decoder.decode(V9Record.self, from: data)
        ...
        for (scenario, record) in allRecords {
            ...
            let isCutMode = input.event_flags.cut_mode ?? false
    ...
    private func computeSessionDate(from input: V9Input, recordDate: String?, calendar: Calendar) -> Date {
        // First try to use the record's date field
    ...
    private func buildUserProfile(from profile: V9UserProfile, isCutMode: Bool) -> UserProfile {
    ...
    private func makeExercise(id: String) -> Exercise {
        ...
        let pattern: MovementPattern = {
            ...
            if lower.contains("ohp") || lower.contains("overhead") || lower.contains("shoulder_press") { return .verticalPush }
            ...
            if lower.contains("tricep") { return .elbowExtension }
        ...
        let equipment: Equipment = {
            if lower.contains("db") || lower.contains("dumbbell") || lower.contains("lateral") { return .dumbbell }
    ...
    private func buildWorkoutHistory(
        ...
        var sessionsByDate: [String: [(lift: String, entry: V9HistoryEntry)]] = [:]
    ...
    private func buildTrainingPlan(from input: V9Input, exerciseCatalog: [String: Exercise]) -> (TrainingPlan, UUID) {
        let templateId = UUID()
        ...
        let templateExercises = input.today_session_template.map { te -> TemplateExercise in
            ...
            // Use smaller increments for isolation exercises
        ...
        if let policyStr = input.config_overrides?.progression_policy?.lowercased() {
            for te in input.today_session_template {
                ...
                case "double_progression":
                    // Use small increment for isolation exercises
    ...
    private func computeReadiness(from metrics: V9TodayMetrics) -> Int {
        var score = 70
    ...
    private func printDiagnostics(
        ...
        let isVerbose = ProcessInfo.processInfo.environment["V9_VERBOSE"] == "1"
        let now = ISO8601DateFormatter().string(from: Date())
        var logLines: [String] = []
        
        logLines.append("=" * 80)
        logLines.append("V9 FULL E2E REPLAY DIAGNOSTICS (conservative, exercise-aware) @ \(now)")
        logLines.append("=" * 80)
        logLines.append("")
        ...
        // Per-experience breakdown (V9 specific)
        logLines.append("-" * 80)
        ...
        // Per-movement-type breakdown (V9 specific - compound vs isolation)
        logLines.append("-" * 80)
        ...
        if isVerbose {
            logLines.append("-" * 80)
            logLines.append("SAMPLE MISMATCHES (up to 100) [V9_VERBOSE=1]:")
            for sample in mismatchSamples.prefix(100) {
                logLines.append("  [\(sample.testId)] \(sample.lift) (\(sample.sex)/\(sample.experience))")
            ...
            logLines.append("-" * 80)
            logLines.append("MISMATCH SAMPLES: \(mismatchSamples.count) total (set V9_VERBOSE=1 for details)")
        }
        ...
        let logURL = URL(fileURLWithPath: FileManager.default.currentDirectoryPath)
            .appendingPathComponent("v9_replay_diagnostics.log")
        do {
            try logText.write(to: logURL, atomically: true, encoding: .utf8)
            print("\n  💾 Saved detailed v9 diagnostics to: \(logURL.path)")
        } catch {
            print("\n  ⚠️ Failed to write v9 diagnostics log: \(error)")
        }
        ...
        ================================================================================
        🧪 workout_engine_testset_v9_conservative_science FULL E2E SCORECARD:
        ================================================================================
